...
/// Enkel loggvy för att fånga noteringar innehållande "Loop slutfördes inte" , inspirerad av BGCheckView.
final class TrioLoopView: ThemedViewController, UITableViewDataSource, UITableViewDelegate {

    // MARK: - Model

    private var entries: [LoopEntry] = []
    ...
    private func focusSelectedDateIfNeeded() {
        guard needsDateFocus, view.window != nil, tableView.bounds.height > 0,
              let index = LoopEntry.nearestIndex(to: selectedDate, in: entries) else { return }
    ...
    func showLoopStats() {
        ...
        let loopDates = entries.map { $0.date }

        let statsVC = TrioLoopStatsViewController(days: days, counts: counts, loopDates: loopDates, now: now)
    ...
    private func setupTableView() {
        ...
        tableView.register(UITableViewCell.self, forCellReuseIdentifier: "LoopCell")
    ...
    /// Hämtar alla `Note`-treatments från cachen vars notes innehåller "Loop slutfördes inte".
    private func loadLoopErrors() {
        ...
        Task {
            ...
            let loopNotes: [LoopEntry] = treatments.compactMap { t -> LoopEntry? in
                guard t.eventType == "Note" else { return nil }

                guard let note = t.notes, note.contains("Loop slutfördes inte") else { return nil }

                let date = t.created_at
                return LoopEntry(date: date, note: note)
            }
            .sorted { $0.date > $1.date }

            await MainActor.run {
                self.entries = loopNotes
    ...
    func tableView(
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "LoopCell") ?? UITableViewCell(style: .subtitle, reuseIdentifier: "LoopCell")
        ...
        guard !entries.isEmpty else {
            cell.textLabel?.text = "Inga loopfel i historiken"
        ...
        // Preserve the complete error reason, wrapping it beside the timestamp.
        let note = entry.note
        let compact = note.replacingOccurrences(of: "\n", with: " ").replacingOccurrences(of: "⚠️ ", with: "")
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let analysisVC = MealAnalysisView(
            ...
            modalTitleString: "Analys loopfel",
